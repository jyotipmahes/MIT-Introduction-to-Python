...
def get_word_score(word, n):
    ...
    sum=0
    for a in word:
        sum=sum+SCRABBLE_LETTER_VALUES[a]
        score=sum*len(word)
    if n==len(word):
        score=score+50
    return score
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while check:
        print("Current Hand:")
        display_hand(hand)
        word=input('Enter Enter word, or a "." to indicate that you are finished:')
        if word=='.':
            check=False
            ...
            print('"'+ word+'" earned '+str(score)+' points. Total score: '+str(Total)+' points.')
